...
Type_ptr get_n_th_from_global(int i) {
  if (strcmp(name_env[i], "Z") == 0 || strcmp(name_env[i], "Y") == 0 ||
...
Type_ptr make_inttype() { return &inttype; }
...
int unify(Type_ptr t1, Type_ptr t2) {
  /* todo */
  Type_ptr new_t1 = simply(t1);
  Type_ptr new_t2 = simply(t2);

  if (new_t1 == NULL || new_t2 == NULL) {
    return 0;
  }
  switch (new_t1->kind) {
  case Int: {
    switch (new_t2->kind) {
    case Typevar: {
      unify_leaf(new_t2, new_t1);
      return 1;
    }
    case Int:
      return 1;
    default: {
      print_error(new_t1, new_t2);
      return 0;
    }
    }
  }
  ...
  case Typevar: {
    switch (new_t2->kind) {
    case Arrow: {
      if (is_occur_node(new_t1->index, new_t2)) {
        printf("Error!!");
        print_error(new_t1, new_t2);
        return 0;
      }
      unify_leaf_arrow(new_t1, new_t2);
      return 1;
    }
    default: {
      unify_leaf(new_t1, new_t2);
      printf("Default");
      return 1;
    }
    }
  ...
  case Arrow: {
    switch (new_t2->kind) {
    case Arrow: {
      if (unify(get_left(new_t1), get_left(new_t2)) &&
          unify(get_right(new_t1), get_right(new_t2)))
        return 1;
      else
        return 0;
    }
    ...
    case Typevar: {
      if (is_occur_node(new_t2->index, new_t1)) {
        print_error(new_t1, new_t2);
        return 0;
      }
      unify_leaf_arrow(new_t2, new_t1);
      return 1;
    }
    }
  }
...
Type_ptr typing(Var_list_ptr abs, AST *t, int top) {
  ...
  case CONST:
    return make_inttype();
  case VAR: {
    ptr_1 = get_n_th(abs, t->value, top);
    break;
  }
  case ABS: {
    ptr_1 = make_vartype();
    ptr_3 = typing(add_var_list(ptr_1, list_copy(abs)), t->rchild, top + 1);
    if (ptr_1 != NULL && ptr_3 != NULL)
      ptr_1 = make_arrowtype(ptr_1, ptr_3);
    else
      ptr_1 = NULL;
    break;
  }
  case COND: {
    ptr_1 = typing(list_copy(abs), t->cond, top);
    ptr_2 = typing(list_copy(abs), t->lchild, top);
    ptr_3 = typing(list_copy(abs), t->rchild, top);
    break;
  }
  case APP: {
    ptr_1 = typing(list_copy(abs), t->lchild, top);
    if (ptr_1 == NULL)
      break;
    ptr_3 = typing(list_copy(abs), t->rchild, top);
    if (ptr_3 == NULL) {
    ...
    ptr_1 = final_type(ptr_1);
    if (ptr_1 != NULL) {
      switch (ptr_1->kind) {
      case Typevar: {
        ptr_1 = get_instance(ptr_1);
        if (unify(ptr_1->left, ptr_3))
          ptr_1 = ptr_1->right;
        else
          ptr_1 = NULL;
        break;
...
int find_old_index(int index) { return index_order[index]; }
...
-> D)) -> ((((E -> F) -> (F -> G)) -> ((E -> F) -> (E -> G))) -> H)) -> H)
((((((A -> B) -> (A -> B)) -> C) -> (((((A -> B) -> (A -> B)) -> C) -> (C -> D))
-> D)) -> ((((E -> F) -> (F -> G)) -> ((E -> F) -> (E -> G))) -> H)) -> H)
((((((A -> B) -> (A -> B)) -> C) -> (((((A -> B) -> (A -> B)) -> C) -> (C -> D))
-> D)) -> ((((E -> F) -> (F -> G)) -> ((E -> F) -> (E -> G))) -> H)) -> H)
((((((A -> B) -> (A -> B)) -> C) -> (((((A -> B) -> (A -> B)) -> C) -> (C -> D))
-> D))E -> ((((F -> G) -> (G -> H)) -> ((F -> G) -> (F -> H)))I -> J)) -> J)
((((((A -> B) -> (A -> B)) -> C) -> (((((A -> B) -> (A -> B)) -> C) -> (C -> D))
-> D)) -> ((((E -> F) -> (F -> G)) -> ((E -> F) -> (E -> G))) -> H)) -> H)
