...
AST *clone_tree(AST *source) {
  while (source != NULL) {
    ...
    case COND:
      return make_cond(clone_tree(source->cond), clone_tree(source->lchild),
                       clone_tree(source->rchild));
    case ABS:
      return make_abs(NULL, clone_tree(source->rchild));
    default:
      return make_app(clone_tree(source->lchild), clone_tree(source->rchild));
    }
  }
  return NULL;
}
...
CLOSURE *clone_clos(CLOSURE *source) {
  if (source == NULL)
    return NULL;
  return make_clos(clone_tree(source->ast), clone_list(source->env),
                   source->index);
}
...
CLOSURE *cbv_primitive(CLOSURE *clos) {
  ...
  CLOSURE *x_arg = get_argument(1, env, 0), *y_arg = get_argument(2, env, 0);
...
CLOSURE *eval_cbv(CLOSURE *clos) {
  ...
  case VAR: {
    if (exp->value == 0 || exp->value < 0)
      return cbv_primitive(clos);
    return get_argument(exp->value, env, index);
  }
  case ABS:
    return clos;
  case COND: {
    CLOSURE *res;
    res = eval_cbv(make_clos(exp->cond, env, index));
    if (res->ast->kind == CONST) {
      if (res->ast->value != 0)
        return eval_cbv(make_clos(exp->lchild, env, index));
      return eval_cbv(make_clos(exp->rchild, env, index));
    }
  }
  case APP: {
    CLOSURE *res1;
    CLOSURE *res2;
    res1 = eval_cbv(make_clos(exp->lchild, env, index));
    res2 = eval_cbv(make_clos(exp->rchild, env, index));
    if (res1->ast->kind == ABS) {
      exp = res1->ast->rchild;
      env = make_list(res2, res1->env);
      return eval_cbv(make_clos(exp, env, res1->index));
    }
  }
  default: {
    printf("Error!!!!!!!");
...
CBN_ENV *eval_cbn(CBN_ENV *clos) {
  ...
  case APP: {
    return eval_cbn(
        make_cbn_env(make_clos(exp->lchild, env, index),
  ...
  case ABS: {
    if (stack != NULL) {
      return eval_cbn(make_cbn_env(
          make_clos(exp->rchild, make_list(stack->clos, env), index),
          stack->next));
      return clos;
    }
  }
  case COND: {
    ...
    if (clos->cbn_env->ast->kind == CONST) {
      ...
      if (value != 0)
        return eval_cbn(
            make_cbn_env(make_clos(exp->lchild, env, index), stack));
  ...
  case VAR: {
    if (exp->value == 0 || exp->value < 0) {
      ...
      return eval_cbn(
          make_cbn_env(get_argument(exp->value, env, index), stack));
    }
  }
  default: {
    printf("Error!!!!!");
